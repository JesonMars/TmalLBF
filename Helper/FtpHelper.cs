...
namespace Helper
{
    ...
    public class FtpHelper
    {
        ...
        /// FTP服务器默认目录  
        /// </summary>  
        private string ftpRemotePath { get; set; }
        /// <summary>  
        /// FTP服务器登录用户名  
        /// </summary>  
        ...
        public FtpHelper(string ftpServerIP, string ftpRemotePath, string ftpUserID, string ftpPassword)
        {
            ...
            this.ftpURI = "ftp://" + ftpServerIP + this.ftpRemotePath;
        ...
        public void AppendFile(byte[] fileInfo, string fileName)
        {
            request = OpenRequest(new Uri(ftpURI + fileName), WebRequestMethods.Ftp.AppendFile);
            request.ContentLength = fileInfo.Length;
            using (var strm = request.GetRequestStream())
            {
                strm.Write(fileInfo, 0, fileInfo.Length);
                strm.Close();
            }
        ...
        /// <param name="fileInfo">文件内容</param>
        /// <param name="fileName">文件名</param>
        public void AppendFile(string fileInfo, string fileName)
        {
            byte[] fileInfoStream = System.Text.Encoding.ASCII.GetBytes(fileInfo.ToCharArray());
            this.AppendFile(fileInfoStream, fileName);
        ...
        /// </summary>  
        /// <returns></returns>  
        public bool IsExistFile(string remoteName)
        {
            return this.ListFiles().IndexOf(remoteName) > -1 ? true : false;
        }
        ...
        public List<string> ListFiles()
        {
            ...
            response = Open(new Uri(ftpURI), WebRequestMethods.Ftp.ListDirectory);
        ...
        #region   未使用过，所以未测的其他功能，包括下载和文件详细列表列举，如需使用请测试后再用
        /// <summary>  
        /// 获取当前目录的文件和一级子目录信息  （未测,下面的逻辑应该是windows的，和linux有区别）
        /// </summary>  
        /// <returns></returns>  
        ...
        //}
        /// <summary>         
        /// 判断当前目录下指定的子目录或文件是否存在     （未测,下面的逻辑应该是windows的，和linux有区别）    
        /// </summary>         
